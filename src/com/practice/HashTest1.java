package com.practice;

import java.util.HashMap;

/**
 * 전화번호부에 적힌 전화번호 중, 한 번호가 다른 번호의 접두어인 경우가 있는지 확인하려 합니다.
 * 전화번호가 다음과 같을 경우, 구조대 전화번호는 영석이의 전화번호의 접두사입니다.
 *
 * 구조대 : 119
 * 박준영 : 97 674 223
 * 지영석 : 11 9552 4421
 * 전화번호부에 적힌 전화번호를 담은 배열 phone_book 이 solution 함수의 매개변수로 주어질 때,
 * 어떤 번호가 다른 번호의 접두어인 경우가 있으면 false를 그렇지 않으면 true를 return 하도록 solution 함수를 작성해주세요.
 */
public class HashTest1 {
    public boolean solution(String[] phone_book) {
        boolean answer = true;

        HashMap<String,Integer> hm = new HashMap<>();

        for(String key: phone_book){
            hm.put(key, Integer.parseInt(key));
        }

        for(String key: hm.keySet()){
            for(String key1: hm.keySet()){
                if(!key.equals(key1) && key.indexOf(key1) == 0) {
                    answer = false;
                    break;
                }
            }
        }

//        for (int i = 0; i < phone_book.length; i++) {
//            for (int j = 0; j < phone_book.length; j++) {
//                System.out.println(i);
//                System.out.println(j);
//                System.out.println(phone_book[i]);
//                System.out.println(phone_book[j]);
//                System.out.println(phone_book[i].indexOf(phone_book[j]));
//                if(i != j && phone_book[i].indexOf(phone_book[j]) == 0) {
//                    answer = false;
//                }
//            }
//        }
//
//        System.out.println(hm);
//
//        Iterator it = hm.entrySet().iterator();
//        while (it.hasNext()) {
//            Map.Entry pair = (Map.Entry) it.next();
//            System.out.println(pair.getKey() + " = " + pair.getValue());
//
//            Iterator it2 = hm.entrySet().iterator();
//            while (it2.hasNext()) {
//                Map.Entry pair2 = (Map.Entry) it2.next();
//
//                if(pair.getKey() != pair2.getKey()){
//                    String value = (String) pair.getValue();
//                    String value2 = (String) pair2.getValue();
//                    if(value.indexOf(value2) > -1) {
//                        answer = false;
//                    }
//                }
//            }
//
//        }



//
//        Iterator it = hm.entrySet().iterator();
//        while (it.hasNext()) {
//            Map.Entry pair = (Map.Entry) it.next();
//            System.out.println(pair.getKey() + " = " + pair.getValue());
//
//            Iterator it2 = hm.entrySet().iterator();
//            while (it2.hasNext()) {
//                Map.Entry pair2 = (Map.Entry) it2.next();
//
//                if(pair.getKey() != pair2.getKey()){
//                    String value = (String) pair.getValue();
//                    String value2 = (String) pair2.getValue();
//                    if(value.indexOf(value2) > -1) {
//                        answer = false;
//                    }
//                }
//
//            }
//
//        }


        return answer;
    }
}